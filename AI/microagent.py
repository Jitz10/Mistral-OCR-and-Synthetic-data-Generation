    ...
    async def calculate_valuation_ratios(self, ticker: str, company_name: str) -> AgentResult:
        ...
        try:
            ...
            for ratio_metric in basic_ratios:
                try:
                    ratio_data = await YFinanceNumberTool.fetch_financial_data(
                        ticker=ticker, 
                        metric=ratio_metric
                    )
                    if ratio_data.get('data'):
                        ratios[ratio_metric.replace('/', '_')] = ratio_data['data'][0].get('value')
            ...
            try:
                ...
                if market_cap_data.get('data') and revenue_data.get('data'):
                    ...
                    if market_cap > 0 and revenue > 0:
                        # Calculate Price-to-Sales using arithmetic tool
                        ps_calculation = ArithmeticCalculationTool.calculate_metrics(
                            inputs={"Market_Cap": market_cap, "Revenue": revenue},
                            formula="Price_to_Sales = Market_Cap / Revenue"
                        )
                        
                        if ps_calculation.get('results'):
                            ratios['Calculated_Price_to_Sales'] = ps_calculation['results'][0].get('value')
                            sources.extend(ps_calculation.get('sources', []))
                ...
                if shares_data.get('data'):
                    ...
                    if shares > 0 and market_cap > 0:
                        # Estimate enterprise value (simplified)
                        ev_calculation = ArithmeticCalculationTool.calculate_metrics(
                            inputs={"Market_Cap": market_cap, "Debt_Factor": 1.1},  # Assume 10% debt premium
                            formula="Enterprise_Value = Market_Cap * Debt_Factor"
                        )
                        
                        if ev_calculation.get('results'):
                            ratios['Estimated_Enterprise_Value'] = ev_calculation['results'][0].get('value')
                            sources.extend(ev_calculation.get('sources', []))
            ...
            try:
                ...
                for chunk in valuation_chunks:
                    ...
                    if content.get('type') == 'text':
                        ...
                        # Use reasoning tool to extract valuation metrics from text
                        reasoning_result = await ReasoningTool.reason_on_data(
                            data=text[:500],
                            query="Extract any valuation ratios (P/E, P/B, EV/EBITDA) mentioned in this text. Provide numerical values if available."
                        )
                        
                        if reasoning_result.get('reasoning'):
                            ratios['Document_Analysis'] = reasoning_result['reasoning'][:200]
                            sources.append('reasoning_tool')
            ...
            if ratios:
                try:
                    valuation_analysis = await ReasoningTool.reason_on_data(
                        data=ratios,
                        query=f"Analyze these valuation ratios for {company_name}. Are they attractive, fair, or expensive? Provide insights."
                    )
                    
                    if valuation_analysis.get('reasoning'):
                        ratios['Valuation_Analysis'] = valuation_analysis['reasoning']
                        ratios['Analysis_Confidence'] = valuation_analysis.get('confidence', 0.5)
                        sources.append('valuation_reasoning')
                except Exception as reasoning_error:
                    logger.warning(f"Valuation reasoning failed: {reasoning_error}")
        ...
        try:
            ...
            for metric in profitability_metrics:
                try:
                    ...
                    if metric_data.get('data'):
                        ratios[metric.replace(' ', '_')] = metric_data['data'][0].get('value')
            ...
            try:
                ...
                if net_income_data.get('data') and total_assets_data.get('data'):
                    ...
                    if net_income != 0 and total_assets != 0:
                        roce_calculation = ArithmeticCalculationTool.calculate_metrics(
                            inputs={"Net_Income": net_income, "Total_Assets": total_assets},
                            formula="ROCE_Simplified = Net_Income / Total_Assets"
                        )
                        
                        if roce_calculation.get('results'):
                            ratios['Calculated_ROCE'] = roce_calculation['results'][0].get('value')
                            sources.extend(roce_calculation.get('sources', []))
                ...
                if net_income_data.get('data') and revenue_data.get('data'):
                    ...
                    if revenue > 0:
                        npm_calculation = ArithmeticCalculationTool.calculate_metrics(
                            inputs={"Net_Income": net_income, "Revenue": revenue},
                            formula="Net_Profit_Margin = Net_Income / Revenue"
                        )
                        
                        if npm_calculation.get('results'):
                            ratios['Calculated_Net_Profit_Margin'] = npm_calculation['results'][0].get('value')
                            sources.extend(npm_calculation.get('sources', []))
            ...
            try:
                ...
                for chunk in profitability_chunks:
                    ...
                    content = chunk.get('content', {})
                    if content.get('text'):
                        profitability_texts.append(content['text'][:300])
                
                # Use reasoning tool to analyze profitability from documents
                if profitability_texts:
                    combined_text = " ".join(profitability_texts)
                    profitability_reasoning = await ReasoningTool.reason_on_data(
                        data=combined_text,
                        query=f"Extract profitability metrics and analyze the profitability trends for {company_name}. Identify key strengths and concerns."
                    )
                    
                    if profitability_reasoning.get('reasoning'):
                        ratios['Document_Profitability_Analysis'] = profitability_reasoning['reasoning']
                        ratios['Analysis_Confidence'] = profitability_reasoning.get('confidence', 0.5)
                        sources.append('profitability_reasoning')
            ...
            try:
                ...
                if historical_data.get('data'):
                    ...
                    trend_analysis = await ReasoningTool.reason_on_data(
                        data=historical_data['data'][:10],  # Last 10 quarters
                        query="Analyze the profitability trends. Is the company becoming more or less profitable over time?"
                    )
                    
                    if trend_analysis.get('reasoning'):
                        ratios['Historical_Trend_Analysis'] = trend_analysis['reasoning']
                        sources.append('historical_trend_analysis')
        ...
        try:
            ...
            for article in news_articles:
                ...
                sentiment_analysis = await ReasoningTool.reason_on_data(
                    data=f"Title: {article['title']}. Summary: {article['summary']}",
                    query="Analyze the sentiment of this news article. Is it positive, negative, or neutral for the company? Provide a confidence score."
                )
            ...
            result_data = {
                ...
                'articles': news_analysis[:5],  # Return top 5 for summary
                'sources': list(set(sources)),
                'timestamp': datetime.now().isoformat()
            }
